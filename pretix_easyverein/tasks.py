...
from .ev_utils import eV_trigger_and_wait_for_onlinebankingimport, eV_get_bankstatements
...
def eV_import():
    ...
    for org in Organizer.objects.all():
        ...
        if org.settings.get('easyverein_import_bankstatements') is None: continue
        ...
        if ev_api_key is None:
            logger.warning(f"{org} has easyverein_import_bankstatements enabled, "
        ...
        if ev_api_key in bankstatements_per_key:
            ...
            eV_trigger_and_wait_for_onlinebankingimport(
                ...
                bankaccount_ids=[id for id in org.settings.get("easyverein_bankaccount_ids").split(",")],
            )
        ...
        logger.info(f"Bank import from easyverein for {org} finished: {reverse('plugins:banktransfer:import.job', kwargs=kwargs)} "
