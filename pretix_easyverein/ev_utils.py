...
def _eV_set_csrfheader_from_response(response: requests.Response, session: requests.Session) -> str:
    csrf_match = re.search(r'<input type="hidden" name="csrfmiddlewaretoken" value="([a-zA-Z0-9]+)">', response.text)
    if csrf_match:
...
def eV_trigger_and_wait_for_onlinebankingimport(short, email, password, bankaccount_ids):
...
def eV_get_bankstatements(api_key, days_back=None):
    ...
    if days_back:
        search_filter = BookingFilter(
            date__gt=(datetime.datetime.now() - datetime.timedelta(days=days_back)).replace(
    ...
    statement = []
    for b in bookings:
        if b.amount and b.amount < 0: continue  # filter outgoing payments
        if b.date is None: continue  # should not happen
        if b.description is None: continue  # can not be processed
